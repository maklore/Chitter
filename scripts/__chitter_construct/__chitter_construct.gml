...
function __chitter() constructor {
	...
	static __part_id  = ds_list_create();
	static __part_system = part_system_create();
	
	part_system_draw_order(__part_system, true);
	part_system_automatic_draw(__part_system, false);
	
	static __font = undefined;
	static __font_name = undefined;
	static __font_sprite = undefined;
	static __font_sprite_struct = {};
	static __font_draw_each = false;
	static __font_scale_base = 1;
	static __font_width_base = 0;
	static __font_height_base = 0;
	static __font_colour_base = 0;
	static __font_colour_base_red   = 0;
	static __font_colour_base_green = 0;
	static __font_colour_base_blue  = 0;
	static __font_colour_base_hue   = 0;
	static __font_colour_base_sat   = 0;
	static __font_colour_base_val   = 0;
	
	static __sound = undefined;
	static __write_pos = 0;
	...
	static __string_length = 0;
	static __string_count = 0;
	...
	*/
	static add = function(_id, _name, _string, _sprite = undefined) {
		...
		if _text_list == undefined or _mod_list == undefined {
			__err_list();
		}
	...
	*/
		    ...
		    for (var ii = 0; ii < _arr_length; ++ii) {
		        ...
		        for (var iii = _index_start; iii < _index_end; ++iii) {
					...
					if _grid[# iii, __chitter_char.line_break] and !_readjust_height {
						...
						__string_current = string_delete(__string_current, iii + 1, 1);
						__string_current = string_insert("\n", __string_current, iii + 1)
						_grid[# iii, __chitter_char.char] = "\n";
	...
	static __text_parse = function(_string) {
		...
		for (var i = 1; i < _string_length; ++i) {
		    ...
		    var _identifier_prev = string_char_at(_string_new, i - 1);
			...
			if _identifier == "[" and _identifier_prev != __chitter_escape_char  {
		        ...
		        for (var ii = i; ii < _string_length; ++ii) {
					...
					if _identifier == __chitter_escape_char {
					...
					if _escape_active == false {
			            ...
			            if _identifier == "]" {
							...
							_string_new = string_delete(_string_new, i, _modifier_list[| _ds_length].length);
