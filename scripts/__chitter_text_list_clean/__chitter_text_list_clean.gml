...
function __text_list_clean(_list) {
	...
	for (var i = 0; i < _ds_length - 1; i += 2;) {
		...
		for (var ii = 0; ii < _arr_length; ++ii;) {
			...
			if _values[ii] == "" { __err(_name, _values[ii]); continue; }
			...
			if _name == "sound_index" or 
				...
				if asset_get_index(_values[ii]) == -1 { __err(_name, _values[ii]); continue; }
		    ...
		    if array_contains(_colours, _name) {
				...
				if string_starts_with(_values[ii], "#") {
					...
					if string_length(_values[ii]) != 7 { __err(_name, _values[ii]); continue; }
					...
					if _real == "" { __err(_name, _real); continue; }
				...
				if string_length(string_letters(_real)) > 0 { __err(_name, _real); continue; }
